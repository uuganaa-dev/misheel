      ...
      <Grid sx={{ width: "100%", height: "100px", fontSize: "15px" }}>
        <HorizontalTimeline
          ...
          indexClick={(index) => {
            ...
            setState({ ...state, curIdx: index, prevIdx: curIdx, values: Timeline[index].text, img: Timeline[index].img });
          }}
          values={Timeline.map((x) => x.year)}
        />
      </Grid>
      <Grid sx={{ display: "flex", gap: "87px", flexDirection: ["column", "row"], alignItems: "center", mt: "36px" }}>
        <Grid
          sx={{
        ...
        />
        <Typography sx={{ fontFamily: "Inter", fontSize: "16px", fontWeight: 300, width: ["71%", "467px"], color: "#404040" }}>
          {state.values}
        </Typography>
      </Grid>
    </Grid>
